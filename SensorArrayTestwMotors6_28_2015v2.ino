...
void setup()
{
  ...
  for (int i = 0; i < 4; i++)  // move back and forth seven times
  {
...
void loop()
{
  ...
  if (position > 2000 && position < 5000) {    //if(the line is visible between sensors 3 and 6, GO)
    //encoder count :: inches = 1000 :: 6.25
    //    if (encoderCountAvg < 5000)    //if(you have travelled less than 5000EU, GO)
    frontSaber.drive(20);
    frontSaber.turn(0);
  ...
  if (position < 2000) {  // if we're not on the line to the right, sweep left
    for (int i = 127; i < 150; i++) {
      commandMotors(127, i, 127, 1);
  ...
  delay(500);
  if (position > 5000) { // if we're not on the line to the left, sweep right
    for (int i = 127; i > 100; i--)
    {
      commandMotors(127, i, 127, 1);
      if (position > 2000 && position < 5000) {
        i = topForwardSpeed;
